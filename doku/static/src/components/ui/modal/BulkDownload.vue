<template>
  <modal ref="modal" title="Bulk Download">
        ...
        <div>
          <div v-for="document in documents" :key="document.id" class="form-group">
            <label class="form-checkbox c-hand">
              <input type="checkbox" :checked="selection.has(document.id)" :disabled="loading" @change="select(document.id, $event)">
              <i class="form-icon" /> {{ document.name }}
            </label>
          </div>
        </div>
    ...
    <div class="modal-footer">
      ...
      <animated-notice ref="downloadNotice" />
