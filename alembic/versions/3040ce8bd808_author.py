...
def upgrade():
    ...
    op.add_column('doku_document', sa.Column('author_id', sa.Integer(), nullable=True))
    op.create_unique_constraint(None, 'doku_document', ['id'])
    op.create_foreign_key(None, 'doku_document', 'doku_user', ['author_id'], ['id'])
    ...
    op.execute(
        documents.update().values({documents.c.author_id: 1})
    )
    op.alter_column(
...
def downgrade():
    ...
    op.drop_column('doku_document', 'author_id')
    # ### end Alembic commands ###
